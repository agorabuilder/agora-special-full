...
class IRtcEngineEventHandler {
 public:
  ...
  @param localProxyIpList local proxy ip list. if not join local proxy, it will be "".
  @param elapsed Time elapsed (ms) from the user calling the \ref IRtcEngine::joinChannel "joinChannel" method until the SDK triggers this callback.
   */
  virtual void onProxyConnected(const char* channel, uid_t uid, PROXY_TYPE proxyType, const char* localProxyIpList, int elapsed) {
    (void)channel;
    (void)uid;
    (void)proxyType;
    (void)localProxyIpList;
    (void)elapsed;
  }
...
class RtcEngineParameters {
 public:
  ...
  int setExternalAudioSource(bool enabled, int sampleRate, int channels) {
    if (enabled)
      return setParameters("{\"che.audio.external_capture\":true,\"che.audio.set_external_source_format\":{\"sampleRate\":%d,\"channelCnt\":%d}}", sampleRate, channels);
    else
      return setParameters("{\"che.audio.external_capture\":false}");
  }
